...
class LineModifier:
    """
    Allows to merge short lines into longer ones provided they are similarly oriented (within threshold).
    ...
    @staticmethod
    def extend_lines(lines_to_extend: list, image: np.ndarray) -> list:
        """
    ...
    @staticmethod
    def merge_lines(lines_to_merge: list) -> list:
        """
        Merges lines provided they are similarly oriented
        :param lines_to_merge: List of lists. Lines to merge
        :return: merged lines
        """
        vertical_lines = LineModifier.discard_not_vertical_lines(lines_to_merge)
        ...
        merged_lines_x = LineModifier.merge_lines_pipeline_2(lines_x)
        merged_lines_y = LineModifier.merge_lines_pipeline_2(lines_y)
    ...
    @staticmethod
    def discard_not_vertical_lines(lines):
        """
        ...
        for line in lines:
            ...
            if angle < LineModifier.angle_thresh:
                continue
    ...
    def merge_lines_pipeline_2(lines):
        ...
        for line in lines:
            ...
            for group in super_lines:
                for line2 in group:
                    ...
                    if LineModifier.get_distance(line2, line) < LineModifier.min_distance_to_merge:
                        # check the angle between lines
                        orientation_i = math.atan2((line[0][1] - line[1][1]), (line[0][0] - line[1][0]))
                        orientation_j = math.atan2((line2[0][1] - line2[1][1]), (line2[0][0] - line2[1][0]))

                        if int(abs(abs(math.degrees(orientation_i)) - abs(
                                math.degrees(orientation_j)))) < LineModifier.min_angle_to_merge:
        ...
        for group in super_lines:
            super_lines_final.append(LineModifier.merge_lines_segments1(group))
    ...
    def get_distance_point_line(px, py, x1, y1, x2, y2):
        ...
        LineMag = LineModifier.get_line_magnitude(x1, y1, x2, y2)
        ...
        if (u < 0.00001) or (u > 1):
            ...
            # // to an endpoint
            ix = LineModifier.get_line_magnitude(px, py, x1, y1)
            iy = LineModifier.get_line_magnitude(px, py, x2, y2)
            ...
            ix = x1 + u * (x2 - x1)
            iy = y1 + u * (y2 - y1)
            distance_point_line = LineModifier.get_line_magnitude(px, py, ix, iy)
    ...
    def get_distance(line1, line2):
        ...
        dist1 = LineModifier.get_distance_point_line(line1[0][0], line1[0][1],
                                                     line2[0][0], line2[0][1], line2[1][0], line2[1][1])
        dist2 = LineModifier.get_distance_point_line(line1[1][0], line1[1][1],
                                                     line2[0][0], line2[0][1], line2[1][0], line2[1][1])
        dist3 = LineModifier.get_distance_point_line(line2[0][0], line2[0][1],
                                                     line1[0][0], line1[0][1], line1[1][0], line1[1][1])
        dist4 = LineModifier.get_distance_point_line(line2[1][0], line2[1][1],
                                                     line1[0][0], line1[0][1], line1[1][0], line1[1][1])
